...
impl LinkedListAllocator {
    ...
    fn find_region(&mut self, size: usize, align: usize) -> Option<(&'static mut ListNode, usize)> {
        // reference to current list node, updated in each iteration
        let mut current = &mut self.head;
        while let Some(ref mut node) = current.next {
            if let Some(alloc_start) = Self::alloc_from_region(&node, size, align) {
    ...
    fn alloc_from_region(region: &ListNode, size: usize, align: usize) -> Option<usize> {
        ...
        if end <= region.end_addr() {
            ...
            // ensure that the excess region is capable of holding ListNode
    ...
    fn size_align(layout: Layout) -> (usize, usize) {
        let layout = layout
            ...
            .expect("adjust alignment failed").pad_to_align();
